...
SELECT COUNT(*) AS total_rows_director_mapping
FROM
 director_mapping;
SELECT COUNT(*) AS total_rows_genre
FROM
 genre;
SELECT COUNT(*) AS total_rows_movie
FROM
 movie;
SELECT COUNT(*) AS total_rows_names
FROM NAMES;
SELECT COUNT(*) AS total_rows_ratings
FROM
 ratings;
SELECT COUNT(*) AS total_rows_role_mapping
FROM
 role_mapping;
...
-- Country has 20 null count, worlwide_gross_income_null_count is 3724, languages_null_count 194, production_company_null_count 528.
...
WHERE YEAR = 2019 OR country IN ('USA', 'India')
GROUP BY g.Genre
...
SELECT COUNT(movie_id) AS total_movies_with_one_genre
...
-- date format in Excel and DB is different.. :(
SELECT 
...
select sum(total_votes) as aggregate_votes, filtered_language
from custom_language_table
where filtered_language IS not null
group by filtered_language;
...
SELECT 
    ...
    CASE
      ...
      WHEN avg_rating between 7 and 7.9 then 'Hit movies'
      WHEN avg_rating between 5 and 6.9 then 'One-time-watch movies'
      else 'Flop movies'
...
SUM(round(AVG(duration))) OVER w AS running_total_duration,
...
AVG(duration) OVER w AS avg_duration;
FROM movie AS m
INNER JOIN
	genre g 
	ON m.id = g.movie_id
WINDOW w AS (partition BY g.genre ROWS UNBOUNDED PRECEDING);
